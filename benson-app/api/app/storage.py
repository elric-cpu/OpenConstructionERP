...
class OperationsStore:
    ...
    def employee_document_upload_context(
        ...
        return {
            ...
            "data_classification": (
                "highly_restricted" if task["requirement_id"] in highly_restricted else "restricted"
            ),
        }
        ...
        *,
        ...
        with self.engine.begin() as db:
            ...
            if task["responsible_party"] not in {"employee", "contractor"}:
                raise InvalidEmployeeTaskTransition("This task is completed by Benson staff")
    ...
    def list_employee_documents(self, employee_id: str) -> list[EmployeeDocumentSummary]:
        with self.engine.connect() as db:
            rows = (
                db.execute(
    ...
    def get_employee_document(self, document_id: str) -> dict[str, Any] | None:
        with self.engine.connect() as db:
            row = (
                db.execute(select(employee_documents).where(employee_documents.c.id == document_id))
                .mappings()
                .first()
            )
        ...
        with self.engine.begin() as db:
            ...
            if decision == "complete":
                ...
                if task["evidence_required"] and not evidence_exists:
                    raise InvalidEmployeeTaskTransition("Required evidence has not been submitted")
                status_value = "completed"
                completed_at = now
                completed_by = actor
            elif decision == "reject":
                if task["status"] != "submitted":
                    raise InvalidEmployeeTaskTransition("Only submitted tasks can be rejected")
