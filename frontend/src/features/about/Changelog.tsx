...
const CHANGELOG: ChangelogEntry[] = [
  { version: '7.10.0', date: '2026-06-13', tag: 'NEW',       summary: 'ERP Chat now renders Markdown tables as real tables instead of raw pipe text, so an assistant answer like a list of projects with budgets reads cleanly, with column alignment, in both the floating chat and the full-page chat. The quick create and guided project setup windows are now fully translated across all 27 languages, with no English left showing in other locales. Regional cost catalogues no longer ship inside the package: they download on demand the first time you import one and are cached locally, so the install is about 15 MB lighter while all thirty regions stay available to import exactly as before and keep working offline once loaded.' },
  { version: '7.9.0', date: '2026-06-13', tag: 'NEW',       summary: 'Your own cost catalogues arrive: keep company price books with their own currency inside the cost database, import them from Excel or CSV including Russian headings and European number formats, filter the cost list by catalogue with one click and export each one back to Excel. Thirty regional reference catalogues now ship inside the install and load offline in seconds. The 5D cost model gains an earned value column with percent-complete recording right in the budget line, schedules generated from an estimate get realistic estimated durations with an Est. badge, administrators can remove the seeded demo projects in one click, and quick create needs only a project name and makes the new project active everywhere immediately. Plus a deep audit pass: European decimal rates import correctly, re-creating a budget never doubles it, change orders in a foreign currency are never blended into a contract total, and the project switcher always keeps up.' },
  { version: '7.8.0', date: '2026-06-12', tag: 'NEW',       summary: 'The bill of quantities gains optional Material, Labor and Equipment columns: a button in the grid settings shows, per position, how the unit rate splits across the three, and the split stays correct right after you edit a resource. Three worked retail-market example projects for German cities are built from real trades with named crews, materials and equipment at regional rates. Consolidated general ledger statements are now limited to administrators, imported validation rules stay separate per project, and the BOQ Excel export reconciles to the grand total again. The AI estimate rejects a broken number before it reaches your money, model validation no longer shows a green pass when nothing was checked, the 3D viewer no longer turns black after several models, and validation messages now appear in your language.' },
  { version: '7.7.0', date: '2026-06-11', tag: 'NEW',       summary: 'Bill of quantities descriptions can now hold a full multi-line specification, the way a German LV Langtext reads, and a toolbar button cycles the row height between compact, comfortable and tall. From the New BOQ window you can import a national standard on the spot, with GAEB, FIEBDC-3 (BC3), Excel and CSV recognised automatically and PDF or CAD routed to the AI import. A worked discount-store example project covers the popular trades from shell and roof through drywall, tiling and painting to CO2 refrigeration, photovoltaics and external works. Partner, country and industry packs now sit under one Packs tab. GAEB X84 import no longer zeros out money and exports valid GAEB DA XML 3.3, large point-cloud uploads finish reliably, and validation now reports honestly on rule sets it does not actually run.' },
  { version: '7.6.0', date: '2026-06-11', tag: 'NEW',       summary: 'A new finance general ledger keeps a chart of accounts, posts balanced journal entries and produces a trial balance, income statement, balance sheet and cash flow. You can erase your own account and data from Settings. The takeoff tool can read a PDF plan with a vision model and propose what it sees. Large CAD and PDF uploads now resume if the connection drops, and limits across uploads, lists and bulk actions are much higher. A new saved-views module remembers a filtered list, and tenant isolation and money math were hardened across twelve modules.' },
