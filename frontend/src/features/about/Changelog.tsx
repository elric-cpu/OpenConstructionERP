...
const CHANGELOG: ChangelogEntry[] = [
  { version: '10.0.0', date: '2026-07-04', tag: 'NEW', summary: 'A milestone release. Onboarding now offers 22 company profiles instead of 9, each mapped to a deliberate module set, and the ready-made pack picker falls back to curated country packs so a clean install always has a one-click language and cost database to load; the Cases hub grows from 4 to 14 worked examples across estimating, tendering, planning, BIM and takeoff, site, quality, commercial and handover, with a category filter, a search box and the same full width as every other page. It also finishes the PDF takeoff quality wave with drawing ergonomics, per-measurement styling and user-defined measurement groups, recognises a point cloud or drone file dropped into project files as a reality-capture asset, renames the Spanish bill of quantities to presupuesto while keeping estimacion for progress payments, and translates the new onboarding and cases strings across all 27 other languages.' },
  { version: '9.9.3', date: '2026-07-04', tag: 'NEW', summary: 'Cost Benchmarks now spans eleven markets (adding France, Netherlands, Spain, Italy, Australia and Canada) and five more building types (data center, laboratory, car park, sports and leisure, senior care home), each region carrying a named public source and a plain-language note on what drives its cost. A new on-page How these benchmarks work section explains the percentile, quartiles, DIN 276 split and confidence, with the full source list.' },
  { version: '9.9.2', date: '2026-07-04', tag: 'FIX', summary: 'A short follow-up that polishes the Cost Explorer and closes a compliance gap. It keeps what you were doing on each tab as you move between By resources, Find work, Compare bases and Substitute, warns in Substitute when the replacement resource is priced per a different unit than the line it replaces, and gives a project in Mexico a dedicated contract-compliance pack for APU, IVA, CFDI and LOPSRM checks.' },
  { version: '9.9.1', date: '2026-07-04', tag: 'NEW', summary: 'A quality release that sharpens the Cost Explorer. Search now understands construction vocabulary, so looking for rebar also finds work priced against reinforcement, and the same applies to the catalog search and to finding work by description. A descriptive, multi-word search returns the closest partial matches instead of dead-ending on zero results. Substitute is steadier: it will not blend in a replacement priced only in another currency, it treats a blank replacement rate as a request to use the catalog price, and it stays well-behaved on extreme figures. The workspace now flags a loaded price base that is not yet indexed for resource search and offers a one-click rebuild, and it keeps that index in step base by base as you import.' },
  { version: '9.9.0', date: '2026-07-03', tag: 'NEW', summary: 'A new Cost Explorer workspace searches the cost and resource databases from the resources you already have: name a material, trade or piece of plant and it finds the priced work that uses them, ranked by how well each work is covered, and it also searches the catalogs for a work by description, compares the same rate code across regional price bases with a warning when the currencies differ, and substitutes one resource inside a work to show the effect on its rate. The starter cost items shipped on a fresh install now carry resource recipes, so the workspace returns real results out of the box before any regional catalogue is imported.' },
  { version: '9.8.0', date: '2026-07-03', tag: 'NEW', summary: 'A field and takeoff release. A new Field Time and Daywork module captures labour and plant hours on site against a project, its resources and its equipment, with cost-code assist and an approval workflow that feeds payroll and earned value, and the About page now shows contributor photos on the community thank-you wall. The in-browser 3D importer adds LightWave (.lwo) meshes, and the PDF takeoff viewer gets a wave of fixes: measurements no longer vanish after changing page or zooming, a per-measurement colour applies on the canvas and in the exported PDF, a count shows its running tally, the current-page thumbnail always loads, you can duplicate a measurement, and new points can snap to the corners of existing ones. It also fixes a header and photo-filter overlap, a date that could shift a day at negative time zones, and database migrations that assumed SQLite syntax.' },
  { version: '9.7.0', date: '2026-07-02', tag: 'NEW', summary: 'A features release: compare two independent DWG drawings side by side, import 3D meshes (glTF, OBJ, DAE, 3DS, STL, PLY, USD) with in-browser quantities, read the dashboard from a redesigned map with a sites and local weather side panel, and support the project through PayPal or GitHub Sponsors with a community thank-you wall. It also loads large BIM models without running out of memory, gives the client portal a Documents area so a shared client can see and open their files, and lets Bill of Quantities quantity cells take imperial feet-and-inches and resolve variable and cross-position references correctly in both metric and imperial so a measured quantity can be reused across positions.' },
  { version: '9.6.1', date: '2026-07-02', tag: 'FIX', summary: 'A fix and hardening release. A photo uploaded from a project Photos tab now becomes a real site picture, so it appears in the Photos tab, the Site Photos gallery, the site diary, the dashboard and the photo strip instead of vanishing, and a shared video now streams and seeks in the file viewer instead of getting stuck on the loading spinner. It also stops a field report from linking a document that belongs to another project, and models ISO 15686-5 residual value in the whole-life cost engine so an asset that outlives the study period is credited rather than written off as scrap. It also completes the Spanish and Mexican Spanish translations, so the newest screens (the 6D whole-life dashboard, the Bill of Quantities resource split, the client portal and cost benchmarks) now read in Spanish instead of falling back to English.' },
  { version: '9.6.0', date: '2026-07-01', tag: 'NEW', summary: 'A change intelligence and 6D lifecycle release. Change Intelligence gains a contractual notice and time-bar register that guards claim and extension-of-time deadlines against your contract standard, a cross-source register of who owes the next action, and change-driver Pareto and run-rate analytics with a burn-rate forecast. The 6D carbon module adds a whole-life dashboard covering embodied and operational carbon by EN 15978 stage and ISO 15686-5 whole-life cost, computed from your BIM model with a dry-run-then-confirm review. It also closes a cross-project read gap on shared saved views and tightens who can edit workspace-level ledger accounts.' },
  { version: '9.5.0', date: '2026-07-01', tag: 'NEW', summary: 'The interface is now available in Mexican Spanish with local construction terminology, a new Contracts workspace tracks parties, guarantees, extension-of-time claims and milestones, and a Mexico pack adds unit-price analysis, IMSS site safety and IVA and CFDI billing. Cross-module Cases playbooks walk through real scenarios end to end, every remaining module gains a built-in How-It-Works guide, and 6D links embodied carbon to BIM model elements. The Bill of Quantities resource split becomes a three-way toggle, the main PDF exports carry your logo, the client portal opens the right page from its sign-in link, the file manager gains a media viewer, and new projects take their currency from your regional preference.' },
  { version: '9.4.0', date: '2026-06-30', tag: 'NEW', summary: 'Imperial now works end to end, including the editable Bill of Quantities grid: quantities, units and rates display in the selected system while storage and the Excel, CSV and GAEB exports stay canonical metric. Whenever a quantity is shown in Imperial the paired per-unit rate is restated reciprocally so the line reconciles and the total stays invariant, the priced PDF reconciles too, and the DWG takeoff, BIM rule and filter reports, bid management, assemblies, match-elements, tendering and AI quick-estimate surfaces all honour the preference.' },
  { version: '9.3.0', date: '2026-06-29', tag: 'NEW', summary: 'A measurement-system and portal-polish release. The Imperial measurement preference is now honoured beyond Takeoff - across read-only quantity displays, the BIM element inspector and its geometry read-outs, and the printed Bill of Quantities report - while Excel, CSV and GAEB exports stay in canonical metric and money is never converted. Project Files now opens a PDF in an inline reader by default (PDF Takeoff stays a separate, explicit choice), Finance gains one-click send-for-approval and reversible invoice status edits, the project photo strip includes image documents, and the new strings are translated across all 26 languages. It also fixes the Archived project view so the toolbar no longer disappears and offers a way back to active projects, adds the Cancelled project status, lets GeoHub anchor a project from its saved address or a manually placed pin, and gives the change-order and field-report dialogs wording that matches the action.' },
  { version: '9.2.0', date: '2026-06-29', tag: 'NEW', summary: 'A BIM viewer, scheduling interoperability and localization release. The model viewer becomes a full federated workspace (open several models in one 3D scene, select and measure, a spatial structure tree, an eye-level Walk mode), filtered model elements turn into an on-screen quantity report or a Bill of Quantities Excel export, and saved views and selection sets get folders. Schedules now export to Microsoft Project XML with import buttons for Microsoft Project XML and XER (.xer), a PDF referenced from a transmittal, inspection or NCR previews inline, procurement gets its own clearly labelled menu area, and the Change Intelligence, reconciliation and approval-route screens are now fully translated across all 26 languages.' },
  { version: '9.1.0', date: '2026-06-29', tag: 'NEW', summary: 'A coordination, projects and notifications release: projects gain working statuses (waiting, on hold, finished) with a colour badge and a status history of who changed what and when, archived projects get their own filterable view with one-click restore, and notification settings become a real per-event, per-channel routing matrix that also drives connected chat integrations (Telegram, Slack, Teams, Discord, WhatsApp), which now actually receive your notifications. It also opens a drafted RFI from the interface, shows discussion author names with a link to the thread from the project page, completes the PDF takeoff reload fixes, and gives the Client and Partner Portal invite a complete sign-in link.' },
  { version: '9.0.1', date: '2026-06-27', tag: 'FIX', summary: 'Opening a BIM file (IFC or RVT) uploaded through Project Files now builds the 3D model on the fly instead of reporting "model not found". The File Manager had been opening such a file by its document id as if it were a model id, so the viewer looked for a model that did not exist yet and never started the conversion; it now hands the document to the same on-demand converter used elsewhere, and a model reached by a stale link is recovered the same way.' },
  { version: '9.0.0', date: '2026-06-26', tag: 'NEW', summary: 'A stability and self-hosting release that hardens the production deployment path. OE_ prefixed environment variables are now honoured, uploaded documents, photos and drawings follow the configured data directory and survive a container restart, the in-app PDF preview and self-hosted fonts are no longer blocked by the content security policy, and an external PostgreSQL database self-heals missing columns and indexes on startup the way the embedded one does. The desktop build now bundles the PDF, computer-vision and point-cloud libraries it was missing, file serving keeps opening files saved by earlier releases, and the database self-heal is safe to run against a live shared database.' },
